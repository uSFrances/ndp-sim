...
def main() -> None:
    ...
    perf_parser.add_argument("--modes", nargs="+")
    perf_parser.add_argument("--focus-edge")
    perf_parser.add_argument("--emit-edge-report", action="store_true")
    ...
    if args.command == "solve-edge":
        ...
        result = solve_edge(
            ...
            bank_interleave_count=(
                solver_cfg.bank_interleave_count(
                    str(payload.get("consumer_op_type", "")),
                    str(payload.get("consumer_port", "")),
                )
                if solver_cfg is not None
                else int(payload.get("bank_interleave_count", 1))
            ),
            producer_port=str(payload.get("producer_port", "")) or None,
            consumer_port=str(payload.get("consumer_port", "")) or None,
            producer_op_type=str(payload.get("producer_op_type", "")) or None,
            consumer_op_type=str(payload.get("consumer_op_type", "")) or None,
        )
    ...
    if args.command == "analyze-performance":
        ...
        payload = analyze_graph_performance(
            ...
            focus_edge=args.focus_edge,
            emit_edge_report=args.emit_edge_report,
        )
    ...
    if args.command == "fill-remapping":
        ...
        if args.dump_solver_results is not None:
            ...
            output_path = fill_external_remapping_file(
                args.path,
                output_path=args.output,
                hw_cfg=hardware,
                solver_cfg=solver_cfg,

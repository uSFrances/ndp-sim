...
def run_transformer_layer(store: TensorStore, layer_id: int, token_num: int):
    ...
    # Qperm: (128, 32, 7, 1), k_slice: (128, 32, 1, 1)
    K_dim = k_slice.shape[0]
    num_slices = 4
    slice_k = K_dim // num_slices
    
    partial_sums_2d = []
    for s_idx in range(num_slices):
        start_k = s_idx * slice_k
        end_k = start_k + slice_k
    ...
    unreduced_qk = np.concatenate(partial_sums_2d, axis=0)

    attn_scores_prefix = f"blk.{layer_id}_node_{layer_id}_attn_scores"
    # 保存原始 matmul 输入以便对比
    save_io_tensor(f"{attn_scores_prefix}_op-mul_mat_in0", k_selected, is_sub_op=False)
    save_io_tensor(f"{attn_scores_prefix}_op-mul_mat_in1", Qperm, is_sub_op=False)
    # 将模拟的局部规约和存为 mul_mat_out
    save_io_tensor(f"{attn_scores_prefix}_op-mul_mat_out", unreduced_qk.astype(np.float32), is_sub_op=False)

    # remote_sum 的输入应与未完全规约中间数据一致；输出仍为规约后的 node_attn_scores（FP32）
    try:
        save_io_tensor(f"{attn_scores_prefix}_op-remote_sum_in0", unreduced_qk.astype(np.float32), is_sub_op=False)
        save_io_tensor(f"{attn_scores_prefix}_op-remote_sum_out", store.get(f"node_{layer_id}_attn_scores"), is_sub_op=False)
    except Exception as e:
        print(f"⚠️ Failed to save remote_sum tensors for {attn_scores_prefix}: {e}")
